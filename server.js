...
app.post('/api/order', (req, res) => {
  ...
  const prices = {
    'Sunset Hostel': { monday: 10, tuesday: 12, wednesday: 11, thursday: 10, friday: 15 },
    'City View Hostel': { monday: 9, tuesday: 11, wednesday: 10, thursday: 9, friday: 14 },
    'Backpackers Haven': { monday: 8, tuesday: 10, wednesday: 9, thursday: 8, friday: 13 },
    'Urban Oasis': { monday: 11, tuesday: 13, wednesday: 12, thursday: 11, friday: 16 }
  };

  // Calculate total price
  let totalPrice = 0;
  if (orderType === 'daily') {
    totalPrice = prices[hostel][days[0]];
  } else {
    totalPrice = days.reduce((total, day) => total + prices[hostel][day], 0);
  }
  ...
  db.query(query, [studentId, hostel, orderType, daysString, totalPrice], (err, results) => {
    if (err) {
